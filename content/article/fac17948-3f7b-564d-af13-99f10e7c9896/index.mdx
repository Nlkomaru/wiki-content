...
title: コマンド一覧
slug: commandlist
uuid: fac17948-3f7b-564d-af13-99f10e7c9896
category: tutorial
...
　  <tr>
      <td>/ticket create <内容></td>
      <td>荒し報告、保護地の相談など、スタッフに報告する時に使用します。例）/ticket create 資源サーバーで○○さんが飛んでいました。調査おねがいします。</td>
    </tr>
  </tbody>
</table>
<br>
...
<tr>
  <td><s>/qs create</s></td>
  <td>ショップを作る際はコマンド以外の操作が必要になります。チェストを置いて売りたいアイテムを右手に持ちます。その状態でスニーク（Shiftキーを押しながら）しながら左クリックします。そしたらチャット欄に値段を入力して送信するだけ！ショップの作成費用は100どんぐりです。ショップ作成時に手に持っていた数を売る個数に設定出来ます。</td>
</tr>
<tr>
  <td>/qs remove</td>
...
<tr>
  <td>/qs limit set <回数></td>
  <td>ショップ利用の回数制限を設定できます。</td>
</tr>
<tr>
  <td>/qs limit period week</td>
  <td>ショップ利用回数制限のリセット周期を設定できます。分/時/日/週/月/年から選択できます。</td>
</tr>
<tr>
  <td>/qs staff list</td>
...
<tr>
  <td>/qs staff add <プレイヤー名></td>
  <td>ショップをいじれる自分以外の人を追加します。共同でのショップ経営に。</td>
</tr>
<tr>
  <td>/qs staff remove <プレイヤー名></td>
...
　  <tr>
      <td>/suke settings<br>/suke toggle</td>
      <td>防具の透明化設定を行います。settingsで、防具4部位の表示・非表示設定を個別に切り替えます。toggleで、防具の透明化機能を一括切替します。<br>※注）胴を非表示にしている間はエリトラを使用する事が出来ません。普段は装備を非表示にしたいけどエリトラが使いたい！という時は一時的にtoggleを使って全表示させましょう。</td>
    </tr>
　  <tr>
...
最終更新日：2025/12/16 sukesukeコマンドを追加
